...
impl DataAccumu for database {
    fn select(
        &self,
        _id: u64,
        _pool: impl sqlx::Database,
    ) -> anyhow::Result<crate::accumu::ObjectDetectionData> {
        todo!()
    }

    fn insert(
        &self,
        _data: crate::accumu::ObjectDetectionData,
        _pool: impl sqlx::Database,
    ) -> anyhow::Result<()> {
        todo!()
    }

    fn delete(&self, _id: u64, _pool: impl sqlx::Database) -> anyhow::Result<()> {
        todo!()
    }
}
...
impl database {
    ...
    async fn insert(
        &self,
        data: crate::accumu::ObjectDetectionData,
        pool: SqlitePool,
    ) -> anyhow::Result<()> {
        sqlx::query!(
            r#"
